...
class ServerClientsInterpolateAggregator(ClientsAvgAggregator):
    ...
    def _para_weighted_avg_interpolate(self, models, recover_fun=None):
        training_set_size = 0
        for i in range(len(models)):
            sample_size, _ = models[i]
            training_set_size += sample_size

        sample_size_global, avg_model_global = models[0]
        sample_size_local, avg_model = models[1]
        for key in avg_model:
            for i in range(len(models)):
                local_sample_size, local_model = models[i]
    ...
    def aggregate(self, agg_info):
        ...
        avg_model_by_interpolate = self._para_weighted_avg_interpolate(global_local_models)
        return avg_model_by_interpolate


class FedOptAggregator(ServerClientsInterpolateAggregator):
